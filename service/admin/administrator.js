  ...
  remarkSystem: function(req, res, next) {
    req.app.db.models.User.find({}, function(err, result){
      for (var i = 0; i < result.length; i ++) {
        if (result[i].status.submitted > 0 && result[i].status.submitted <= 5 && result[i].rankingScore < 1)
          req.app.db.models.User.findByIdAndUpdate(result[i]._id, { "badge": 'Chihuahua'}, function(err, result){
            if (err)
              return err;
          });
        else if ((result[i].status.submitted > 5 && result[i].status.submitted <= 10) || result[i].rankingScore == 1)
          req.app.db.models.User.findByIdAndUpdate(result[i]._id, { "badge": 'Puggle'}, function(err, result){
            if (err)
              return err;
          });
        else if ((result[i].status.submitted > 10 && result[i].status.submitted <= 20) || (result[i].rankingScore > 1 && result[i].rankingScore < 5))
          req.app.db.models.User.findByIdAndUpdate(result[i]._id, { "badge": 'Boxer'}, function(err, result){
            if (err)
              return err;
          });
        else if ((result[i].status.submitted > 20) || (result[i].rankingScore > 4 && result[i].rankingScore < 10))
          req.app.db.models.User.findByIdAndUpdate(result[i]._id, { "badge": 'Golden Retriever'}, function(err, result){
            if (err)
              return err;
          });
        else if (result[i].rankingScore > 9)
